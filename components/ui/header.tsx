...
import { getNavigationForHeader } from '@/content/queries';

export default async function Header() {
  const data = await getNavigationForHeader();
  const navItems = data.navigationCollection.items[0].linksCollection
        ...
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <nav className="hidden md:flex md:grow">
            ...
            <ul className="flex grow justify-center flex-wrap items-center">
              {navItems.items.map((nav, index) => (
              <li key={index}>
                <Link className="font-medium text-sm text-slate-300 hover:text-white mx-4 lg:mx-5 transition duration-150 ease-in-out" href={nav.link}>{nav.label}</Link>
              </li>
              ))}
            </ul>
